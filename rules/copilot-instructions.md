...
The louder the cry, the more the line wants a human's eyes at review — every `FORCE-EVOLVED` is the grep anchor that surfaces the override before merge. An evolve covers **one construct, not the session**: the Splash test resumes on the very next construct.
